...
public class ExportServiceImpl implements ExportService {
    ...
    private int createInvoiceSection(Sheet sheet, Invoice invoice, double rowNum) {

        CellStyle leftAlignStyle = sheet.getWorkbook().createCellStyle();
        leftAlignStyle.setAlignment(HorizontalAlignment.LEFT);

        Row invoiceRow = sheet.createRow((int) rowNum++);
        ...
        for (int j = 0; j < invoice.getInvoiceProducts().size(); j++) {
            ...
            if (j > 0) {
                invoiceRow = sheet.createRow((int) rowNum++);
            }
