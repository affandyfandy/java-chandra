...
- `Class Declaration and Fields:`
  - The Employee class is marked with @Component, indicating it's a Spring-managed component and eligible for auto-detection and auto-configuration.
  - It has private fields id, name, age, and employeeWork.
    - id is initialized to 1.
    - name is initialized to "John Doe".
    - age is initialized to 30.
  - employeeWork is marked as final, indicating it's initialized once and cannot be changed afterwards.
- `Setter Injection:`
  - The class uses setter injection with @Autowired. This means when an Employee instance is created, Spring will automatically inject an instance of EmployeeWork into it.
    - @Autowired on the setEmployeeWork method allows Spring to inject the EmployeeWork dependency.
    - Setter injection is used to set the employeeWork field after the Employee object is created.
- `working() Method:`
  - This method prints details of the Employee object (id, name, age) using the overridden toString() method.
  - It then calls the work() method on the injected EmployeeWork instance (employeeWork). This demonstrates collaboration between Employee and EmployeeWork components.
- `toString() Method:`
  ...
  - It returns a string in the format "Employee [id=" + id + ", name=" + name + ", age=" + age + "]", showing the id, name, and age of the employee.
...
- `Field Injection:`
  - The class uses field injection with @Autowired for the employeeWork field.
    - @Autowired on the employeeWork field allows Spring to inject an instance of EmployeeWork directly into this field when creating an Employee instance.
    - Field injection is straightforward and allows dependencies to be injected directly into fields without the need for setter methods or constructors.
