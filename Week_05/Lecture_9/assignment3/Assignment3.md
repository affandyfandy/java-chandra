...
`Objective: `Add Feature for Generate PDF and Calculate Bind Data.
...
- Create [`employee Model`](src/main/java/org/assignment1/assignment3/model/Employee.java) to receive employee entities (id,firstname,lastname,email, etc)
...
- Create [EmployeeRepository](src/main/java/org/assignment1/assignment3/repository/EmployeeRepository.java) for providing JPA Spring data
...
- Create [`EmployeeService`](src/main/java/org/assignment1/assignment3/service/EmployeeService.java) and [`EmployeeServiceImpl`](src/main/java/org/assignment1/assignment3/service/impl/EmployeeServiceImpl.java) to defining the contract for managing Employee entities. Implementations of EmployeeService would typically interact with a repository (e.g., EmployeeRepository) to perform such as `findAll`, `findBy` operations on the database.
  ...
  - `Optional<Employee> getHighestSalaryEmployee()`
    ...
    - Purpose: finds the employee with the highest salary from the list of all employees.
    - Return :
      - If there is at least one employee, it returns an Optional containing the employee with the highest salary.
      - If there are no employees, it returns an empty Optional.
    - Usage: Used this method when need to find out which employee has the highest salary in your database.

    <br>

  - `Optional<Employee> getLowestSalaryEmployee()`

    - Purpose: finds the employee with the lowest salary from the list of all employees.
    - Return :
      - If there is at least one employee, it returns an Optional containing the employee with the lowest salary.
      - If there are no employees, it returns an empty Optional.
    - Usage: Used this method when need to find out which employee has the lowest salary in your database.

    <br>

  - `long getRecordCount()`

    - Purpose: finds employees total record.
    - Return : The method returns a long value representing the total count of employee records.
    - Usage: Use this method when need to know how many employees are currently stored in the database.

    <br>

  - `OptionalDouble getAverageSalary()`
    - Purpose: finds the average of all the employees on the database list.
    - Return :
      - If there are employees, it returns an OptionalDouble containing the average salary.
      - If there are no employees, it returns an empty OptionalDouble.
    - Usage: Used this method when need to find out the average salary of all employees in the database.

<br>

- Create [`CSVUtils`](src/main/java/org/assignment1/assignment3/utils/CSVUtils.java) and [`DateUtils`](src/main/java/org/assignment1/assignment3/utils/DateUtils.java) to manage di csv and date parsing date from data `csv`
...
- Create the [`EmployeeController`](src/main/java/org/assignment1/assignment3/controller/EmployeeController.java) that uses EmployeeService to perform CRUD operations on Employee entities. EmployeeController class is a Spring MVC controller that handles web requests related to employee management.
  ...
  - Dependency Injection
    ...
    - private final PdfService pdfService:
      - Dependency injected via constructor (due to @AllArgsConstructor).
      - This service is used to handle business logic related to PdfService.
  ...
  - Request Handlers
    ...
    - @GetMapping("/pdf")
      - Purpose: The purpose of this method is to generate a PDF document containing information about all employees and return it as a downloadable file in the HTTP response. This allows users to download or view a dynamically generated report of the employee data.
      - Process:
        - Retrieve All Employees
        - Generate PDF:
          - A variable pdfContents of type byte[] is initialized to null.
          - The generatePdf method of the PdfService is called with the list of employees as an argument. This method processes the employee data and generates a PDF.
          - The resulting PDF content is stored in the pdfContents variable.
          - If an exception occurs during PDF generation, it is caught, and the stack trace is printed to the console.
        - Prepare HTTP Headers:
          - The content type of the response is set to MediaType.APPLICATION_PDF to indicate that the response body will contain a PDF document.
          - The Content-Disposition header is set to inline with the filename "employees.pdf". This suggests that the PDF should be displayed in the browser if possible, or downloaded with the given filename.
        - Return Response
...
- Create the page to display [`list page`](src/main/resources/templates/list-employees.html) of employees and page [`form`](src/main/resources/templates/employee-form.html) to update and add employees.
  ...
  - Display to add employee and the result after
    ...
    ![Alt text](img/3.5.png) ![Alt text](img/3.6.png)

  <br>

  - Display after Delete one employee

    ![Alt text](img/3.7.png) ![Alt text](img/3.8.png)

  <br>

  - Display form Update and after update

    ![Alt text](img/3.9.png) ![Alt text](img/3.10.png)
...
- Create employee button via `csv` on [`list page`](src/main/resources/templates/list-employees.html)

  ![Alt text](img/3.12.png) ![Alt text](img/3.13.png) ![Alt text](img/3.14.png) ![Alt text](img/3.15.png)

- Create a [`Generate PDF`](src/main/resources/templates/pdf-employees.html) button to generate PDF with template.

  ![Alt text](img/3.16.png) ![Alt text](img/3.17.png) ![Alt text](img/3.18.png)
